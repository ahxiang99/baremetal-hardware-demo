...
int main() {
  ...
  while (1) {
    ...
    snprintf(buf, sizeof(buf), "20%02d/%02d/%02d %02d:%02d:%02d", d.Year,
             d.Month, d.Day, t.Hours, t.Minutes, t.Seconds);
    g.disp.Show(buf, 0, 0, 0);

    const AppMode curMode = g_AppMode.load(std::memory_order_relaxed);
    snprintf(buf, sizeof(buf), "AppMode: %s",
             namingTable[static_cast<uint8_t>(curMode)].name);
    ...
    if constexpr (kSensorEnable) {
      ...
      if constexpr (kOxiMeterEnable) {
        ...
        cmd_queue.push({[](void *ctx) { static_cast<Max30102 *>(ctx)->read(); },
                        &oxiSensor});
        char buf[128];
        if (oxiSensor.isDataReady()) {
          ...
          snprintf(buf, sizeof(buf), "BPM: %d, SpO2: %d.%d",
                   oxiSensor.getData().bpm, SpO2_v.Integer, SpO2_v.Decimal);
          g.disp.Show(buf, 0, 8, 1);
      ...
      case AppMode::Console: {
        if (g.my_systick.get_ticks() - measure_start > 150) {
          FloatIntExtraction temp =
              convertInt(temp_sensor.getValue().temperature);
          snprintf(buf, sizeof(buf), "SHT40: Temp:%02d.%02d", temp.Integer,
                   temp.Decimal);
          g.disp.Show(buf, 0, 16, 2);
          temp = convertInt(temp_sensor.getValue().humidity);
          snprintf(buf, sizeof(buf), "SHT40: Rh:%02d.%02d", temp.Integer,
                   temp.Decimal);
          g.disp.Show(buf, 0, 24, 3);
          temp = convertInt(stts_temp.getTemp());
          snprintf(buf, sizeof(buf), "STTS2H: Temp:%02d.%02d", temp.Integer,
                   temp.Decimal);
...
extern "C" void TIM3_IRQHandler(void) { getDrivers().timer.handleInterrupt(); }
