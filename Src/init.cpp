...
namespace
{
  ...
  [[noreturn]] void panic(BootTag tag, uint32_t err)
  {
    ...
    _PWR->CR |= PWR_CR_DBP;
    RTC->BKP0R[5]
      = 0xDEAD0000u | (static_cast<uint32_t>(tag) << 8) | (err & 0xFF);
    ...
    constexpr uint32_t DOT = 150'000, DASH = 450'000, GAP = 450'000,
                       WORD_GAP = 2'000'000;
    while(true)
      {
        for(int i = 0; i < 3; ++i)
  ...
  void enableFpu()
  {
    ...
    static volatile uint32_t &CPACR
      = *reinterpret_cast<volatile uint32_t *>(0xE000ED88);
    CPACR |= ((3UL << 20) | (3UL << 22));
  ...
  void initCore(Drivers &g)
  {
    ...
    g.my_systick.init();
    LOG_INFO("AHB: {}, APB1: {}, APB2: {}", board::clock_tree.ahb,
             board::clock_tree.apb1, board::clock_tree.apb2);
  }

  void initComms(Drivers &g)
  {
    checkOk(g.uart2.initialize(board::uart2::cfg, board::uart2::hdmatx_cfg,
  ...
  void initGpioMux()
  {
    ...
    static constexpr const GPIO_Config *mux[] = {
      &board::uart2::gpio_cfg, &board::i2c1::gpio_cfg, &board::spi1::gpio_cfg};
    for(auto *cfg : mux)
      {
  ...
  void initRtc(Drivers &g)
  {
    ...
    if(!g.rtc.IsInit())
      {
        RTC_TimeTypeDef t{
          .Hours = 15, .Minutes = 53, .Seconds = 0, .TimeFormat = 0};
