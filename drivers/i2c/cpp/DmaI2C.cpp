...
Result<> DmaI2C::initialize(const i2c_config_t &cfg, const DMA_Config &hdmatx_cfg,
			    const DMA_Config &hdmarx_cfg)
{
	hdmatx.setConfig(hdmatx_cfg);
	hdmarx.setConfig(hdmarx_cfg);
...
bool DmaI2C::Read(uint16_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
	LOG_DEBUG("Read() called: state={} SR2={}",
		  static_cast<uint32_t>(state_.load(std::memory_order_relaxed)), (Hex)i2c_->SR2);
...
bool DmaI2C::MemWrite(uint16_t DevAddress, uint8_t MemAddr, uint8_t *pData, uint16_t Size,
		      uint32_t Timeout)
{
	if (state_.load(std::memory_order_relaxed) != i2c_state_t::READY) {
		return false;
...
bool DmaI2C::MemRead(uint16_t DevAddress, uint8_t MemAddr, uint8_t *pData, uint16_t Size,
		     uint32_t Timeout)
{
	if (state_.load(std::memory_order_relaxed) != i2c_state_t::READY) {
		return false;
...
void DmaI2C::handleMasterEventInterrupt(const uint32_t &sr1, const uint32_t &sr2,
					const i2c_state_t &snap)
{
	...
	if (sr1 & I2C_SR1_ADDR) {
		if (snap == i2c_state_t::BUSY_RX && rxDmaPending_.load(std::memory_order_acquire)) {
			rxDmaPending_.store(false, std::memory_order::relaxed);
			if (XferSize == 1) {
				// RM0368 §18.3.8: 1-byte DMA receive is broken — LAST sends NACK
				// after the DMA reads DR, but by then the slave was already ACK'd
				// on byte 1's 9th clock and begins sending byte 2, leaving RXNE=1
				// stuck and the bus hung. Fix: abandon DMA, disable ACK now, queue
				// STOP, read via RXNE interrupt.
				push_isr_event(I2C_IsrTag::EV_ADDR_RX_1BYTE);
				disable_DMA_request();
				disableAckBit();
				I2C_CLEAR_ADDR_(
					i2c_);   // release clock stretch before programming STOP
				...
				hdmarx.StartDataStream(reinterpret_cast<uint32_t>(&i2c_->DR),
						       ...
						       XferSize);
				I2C_CLEAR_ADDR_(i2c_); // must happen after DMA armed
			}
		} else {
			push_isr_event(I2C_IsrTag::EV_ADDR_TX);
			I2C_CLEAR_ADDR_(i2c_);
		}
		return;
	}
	...
	if (sr1 & I2C_SR1_RXNE) {
		if (snap == i2c_state_t::BUSY_RX &&
			...
			rxBuffer[0] = static_cast<uint8_t>(
				i2c_->DR); // STOP already queued in ADDR handler
		...
		if (snap == i2c_state_t::BUSY_TX) {
			if (dma_complete_.load(std::memory_order::relaxed)) {
				if (mode_ == i2c_mode_t::MEM_READ) {
					// MEM_READ: register address byte sent — issue repeated
					// START to switch to RX
					push_isr_event(I2C_IsrTag::EV_MEM_RESTART, MemAddr_);
					state_.store(i2c_state_t::BUSY_RX,
					...
					if (XferSize == 0) {
						disableInterrupt();
						generate_stop();
						state_.store(i2c_state_t::READY,
							     std::memory_order_relaxed);
						mode_ = i2c_mode_t::NONE;
					} else {
						i2c_->DR = *XferPtr;
