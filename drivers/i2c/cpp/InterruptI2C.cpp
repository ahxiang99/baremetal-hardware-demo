...
bool InterruptI2C::Write(uint16_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
...
void InterruptI2C::handleEVInterrupt()
{
	...
	else if ((sr1 & I2C_SR1_TXE) && !(sr1 & I2C_SR1_BTF) && (state_ == i2c_state_t::BUSY_TX)) {
		if (XferSize > 0) {
			i2c_->DR = *XferPtr++;
			XferSize--;
		}
...
{
	RegisterUtils::setBits(i2c_->CR2, I2C_CR2_ITEVTEN | I2C_CR2_ITBUFEN | I2C_CR2_ITERREN);
}
