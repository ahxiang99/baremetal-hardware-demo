...
namespace
{
const tim_info_t tim_table[static_cast<uint8_t>(TimerDevice_t::TIMER_COUNT)] = {
...
void Stm32Timer::configureTimer(const TimerConfig &cfg)
{
    ...
    // CMS
    if (cfg.AlignedMode != TimerCenterAlignedMode_t::EDGE) {
	RegisterUtils::modify(temp, TIM_CR1_CMS_Msk, static_cast<uint8_t>(cfg.AlignedMode) << TIM_CR1_CMS_Pos);
    }

    // Up or Down Counter
    if (cfg.Direction == TimerDirection_t::DOWN) {
	RegisterUtils::setBits(temp, TIM_CR1_DIR);
    }

    // ARR
    if (cfg.AutoReloadPreload == TimerARR_t::ENABLE) {
	RegisterUtils::setBits(temp, TIM_CR1_ARPE);
    }
...
TIM_TypeDef *Stm32Timer::getInstance() const
{
    return timer_instance_;
}
