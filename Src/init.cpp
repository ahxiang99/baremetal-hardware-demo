...
enum class BootTag : uint8_t { Clock = 1, GpioMux, Led, Uart2, I2c1, Exti, Rtc, Spi1, Oled };
...
void checkOk(const Result<>& r, BootTag what) {
    if (!r.isOk()) panic(what, static_cast<uint32_t>(r.error()));
}
...
void initComms(Drivers& g) {
    checkOk(g.uart2.initialize(board::uart2::cfg, board::uart2::hdmatx_cfg, board::uart2::hdmarx_cfg), BootTag::Uart2);
    Logger::Init(UartRef::from(g.uart2));
    Logger::set_level(LogLevel::INFO);
    LOG_INFO("UART2 up");

    g.i2c1.configure(board::i2c1::cfg);
    checkOk(g.i2c1.initialize(), BootTag::I2c1);
    LOG_INFO("I2C1 up");
}

void initGpioMux() {
    /* this is for peripherals gpio */
    static constexpr const GPIO_Config* mux[] = {&board::uart2::gpio_cfg, &board::i2c1::gpio_cfg, &board::spi1::gpio_cfg};
    for (auto* cfg : mux) {
