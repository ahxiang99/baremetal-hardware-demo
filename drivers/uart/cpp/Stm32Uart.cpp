...
namespace
{
static const peripherals_regs_table<USART_TypeDef> uart_table[static_cast<uint8_t>(
	UartDevice_t::USART_COUNT)] = {{USART1, RCC_APB2ENR_USART1_EN, RCC_APB2RSTR_USART1_RST},
...
USART_TypeDef *enableAndGet(UartDevice_t dev)
{
	...
	RegisterUtils::setBits(entry.instance == USART2 ? RCC->APB1ENR : RCC->APB2ENR,
			       entry.enableBit);
	(void)RCC->APB1ENR;
	(void)RCC->APB2ENR;
...
bool Stm32Uart::send(std::span<const uint8_t> data)
{
...
bool Stm32Uart::receive(std::span<uint8_t> buf)
{
...
void Stm32Uart::setBaudRate(UartBaudRate_t baudrate)
{
	...
	uint32_t fck = (uart_ == USART2) ? getDrivers().sysclock.getSysClock().apb1
					 : getDrivers().sysclock.getSysClock().apb2;
...
void Stm32Uart::setStopBits(UartStopBit_t stopbit)
{
	...
	case UartStopBit_t::USART_CR2_STOP_2:
		RegisterUtils::setBits(uart_->CR2,
				       static_cast<uint8_t>(UartStopBit_t::USART_CR2_STOP_2) << 12);
		break;
	}
}
