...
Result<> DmaI2C::initialize(const i2c_config_t &cfg, const DMA_Config &hdmatx_cfg, const DMA_Config &hdmarx_cfg)
{
...
void DmaI2C::handleMasterEventInterrupt(const uint32_t &sr1, const uint32_t &sr2, const i2c_state_t &snap)
{
	...
	if (snap == i2c_state_t::BUSY_RX && rxDmaPending_.load(std::memory_order_acquire)) {
		...
		disable_DMA_request();
		disableAckBit();
		I2C_CLEAR_ADDR_(i2c_); // release clock stretch before programming STOP
...
void DmaI2C::handleMemEventInterrupt(const uint32_t &sr1, const uint32_t &sr2, const i2c_state_t &snap)
{
    push_isr_event(I2C_IsrTag::EV_ENTRY, sr1, sr2);
    // ── SB: Start  generated ────────────────────────────────────────
    if (sr1 & I2C_SR1_SB) {
