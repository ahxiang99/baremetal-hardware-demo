...
void WindowWatchDog::initialize(uint32_t apb1Hz) {
    ...
    const float_t decrementsToReset = static_cast<float_t>(RELOAD_VALUE + 1U - CRITICAL_VALUE);
    t_max = (4096.0f * static_cast<float_t>(1U << PRESCALER) * decrementsToReset /
             static_cast<float_t>(apb1Hz)) *
            1000.0f;
