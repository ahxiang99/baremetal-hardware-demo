...
static const i2c_table_info i2c_table[static_cast<uint8_t>(
    i2c_device_t::TotalNum)]{{I2C1, RCC_APB1ENR_I2C1_EN, RCC_APB1RSTR_I2C1_RST,
...
bool Stm32I2C::Write(uint16_t DevAddress, const uint8_t *pData, uint16_t Size,
                     uint32_t Timeout) {
  return WriteTo7BitDevice(DevAddress, pData, Size, Timeout);
}
bool Stm32I2C::Read(uint16_t DevAddress, uint8_t *pData, uint16_t Size,
                    uint32_t Timeout) {
  return ReadFrom7BitDevice(DevAddress, pData, Size, Timeout);
}

bool Stm32I2C::WriteTo7BitDevice(uint16_t DevAddress, const uint8_t *pData,
  ...
  if (state_ == i2c_state_t::READY) {
    ...
    if (WaitForFlagTimeout(i2c_->SR1, I2C_SR1_SB, Timeout)) {
      ...
      error_ = i2c_error_t::ERR_I2C_SB; /* Cannot generate start  */
      return false;
    }
    ...
    while (Size > 0U) {
      ...
      if (WaitForFlagTimeout(i2c_->SR1, I2C_SR1_TXE, Timeout)) {
        state_ = i2c_state_t::READY;
        mode_ = i2c_mode_t::NONE;
        error_ = i2c_error_t::ERR_I2C_TXE;
        return false;
      }
      i2c_->DR = *pData;
      pData++;
      ...
      if (WaitForFlagTimeout(i2c_->SR1, I2C_SR1_BTF, Timeout)) {
        state_ = i2c_state_t::READY;
        mode_ = i2c_mode_t::NONE;
        error_ = i2c_error_t::ERR_I2C_BTF;
        return false;
      }
    }
    generate_stop();
...
bool Stm32I2C::ReadFrom7BitDevice(uint16_t DevAddress, uint8_t *pData,
                                  uint16_t Size, uint32_t Timeout) {
  if (state_ == i2c_state_t::READY) {
    if (isHardwareBusy(Timeout)) {
      error_ = i2c_error_t::ERR_I2C_BUSY;
      ...
      if (Size <= 3U) {
        if (Size == 1) {
          if (WaitForFlagTimeout(i2c_->SR1, I2C_SR1_RXNE, Timeout)) {
            state_ = i2c_state_t::READY;
            mode_ = i2c_mode_t::NONE;
            error_ = i2c_error_t::ERR_I2C_RXNE;
            return false;
          }
          *pData = i2c_->DR;
          pData++;
...
void Stm32I2C::disable_i2c() {
  RegisterUtils::clearBits(i2c_->CR1, I2C_CR1_PE);
}

void Stm32I2C::enable_i2c() { RegisterUtils::setBits(i2c_->CR1, I2C_CR1_PE); }

void Stm32I2C::set_addressing_mode(const i2c_addressmode_t &addr) {
  switch (addr) {
...
void Stm32I2C::set_freq() {
  i2c_->CR2 = getDrivers().sysclock.getSysClock().apb1 / 1'000'000;
}

void Stm32I2C::set_ccr_and_trise(const i2c_freq_t &cfg) {
  /* Configrue CCR */
  uint32_t SCL_Freq = 1;
  switch (cfg) {
...
void Stm32I2C::Error_Handler() {
  ...
  LOG_ERROR("I2C Error: {}",
            static_cast<uint16_t>(error_.load(std::memory_order_relaxed)));
...
void Stm32I2C::generate_start() {
  RegisterUtils::setBits(i2c_->CR1, I2C_CR1_START);
}
void Stm32I2C::generate_stop() {
  RegisterUtils::setBits(i2c_->CR1, I2C_CR1_STOP);
}
...
void Stm32I2C::clear_addr() {
  uint32_t temp;
  temp = i2c_->SR1;
  temp = i2c_->SR2;
  (void)(temp);
}
void Stm32I2C::clear_nack() { RegisterUtils::clearBits(i2c_->SR1, I2C_SR1_AF); }
void Stm32I2C::clear_berr() {
  RegisterUtils::clearBits(i2c_->SR1, I2C_SR1_BERR);
}
void Stm32I2C::clear_arlo() {
  RegisterUtils::clearBits(i2c_->SR1, I2C_SR1_ARLO);
}
void Stm32I2C::clear_ovr() { RegisterUtils::clearBits(i2c_->SR1, I2C_SR1_OVR); }
void Stm32I2C::enable_nvic(const i2c_device_t &cfg) {
  My_NVIC_EnableIRQ(i2c_table[static_cast<uint8_t>(cfg)].irqNumEv);
  My_NVIC_EnableIRQ(i2c_table[static_cast<uint8_t>(cfg)].irqNumEr);
}
