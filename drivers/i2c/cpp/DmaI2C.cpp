...
Result<> DmaI2C::initialize(const i2c_config_t &cfg) {
...
bool DmaI2C::Write(uint16_t DevAddress, const uint8_t *pData, uint16_t Size,
                   uint32_t Timeout) {
  if (state_.load(std::memory_order::relaxed) != i2c_state_t::READY) {
    return false;
  }
  ...
  if (Size == 0 || pData == nullptr) {
    error_ = i2c_error_t::ERR_I2C_DATA_EMPTY;
    return false;
  }
  ...
  XferSize = Size;
  XferLength = XferSize;
  state_.store(i2c_state_t::BUSY_TX, std::memory_order_release);
  mode_ = i2c_mode_t::MASTER;
  error_.store(i2c_error_t::NONE, std::memory_order_relaxed);
...
bool DmaI2C::Read(uint16_t DevAddress, uint8_t *pData, uint16_t Size,
                  uint32_t Timeout) {
  LOG_DEBUG("Read() called: state={} SR2={}",
            static_cast<uint32_t>(state_.load(std::memory_order_relaxed)),
            (Hex)i2c_->SR2);
  if (state_.load(std::memory_order_relaxed) != i2c_state_t::READY) {
    return false;
  }
  ...
  mode_ = i2c_mode_t::MASTER;
  state_.store(i2c_state_t::BUSY_RX, std::memory_order::release);
...
bool DmaI2C::MemWrite(uint16_t DevAddress, uint8_t MemAddr, uint8_t *pData,
  ...
  XferSize = Size;
  XferLength = XferSize;
  mode_ = i2c_mode_t::MEM_WRITE;
  state_.store(i2c_state_t::BUSY_TX, std::memory_order_release);
  error_.store(i2c_error_t::NONE, std::memory_order_relaxed);
  dma_complete_.store(
      true, std::memory_order_relaxed); // pre-set: no DMA for mem addr byte
...
bool DmaI2C::MemRead(uint16_t DevAddress, uint8_t MemAddr, uint8_t *pData,
                     uint16_t Size, uint32_t Timeout) {
  if (state_.load(std::memory_order_relaxed) != i2c_state_t::READY)
    return false;
  if (isHardwareBusy(Timeout)) {
...
void DmaI2C::handleMasterEventInterrupt(const uint32_t &sr1,
                                        const uint32_t &sr2,
                                        const i2c_state_t &snap) {
  push_isr_event(I2C_IsrTag::EV_ENTRY, sr1, sr2);

  // ── SB: Start  generated ────────────────────────────────────────
  if (sr1 & I2C_SR1_SB) {
    push_isr_event(I2C_IsrTag::EV_SB, static_cast<uint32_t>(snap), DevAddr);
    if (snap == i2c_state_t::BUSY_TX) {
      i2c_->DR = DevAddr & ~(1u << 0); // write address
    } else if (snap == i2c_state_t::BUSY_RX) {
      i2c_->DR = DevAddr | (1u << 0); // read address
    } else {
      generate_stop();
      error_.store(i2c_error_t::ERR_I2C_SB, std::memory_order::relaxed);
      state_.store(i2c_state_t::READY, std::memory_order::relaxed);
    }
    return;
  }
  ...
  if (sr1 & I2C_SR1_ADDR) {
    if (snap == i2c_state_t::BUSY_RX &&
        rxDmaPending_.load(std::memory_order_acquire)) {
      rxDmaPending_.store(false, std::memory_order::relaxed);
      if (XferSize == 1) {
        // RM0368 §18.3.8: 1-byte DMA receive is broken — LAST sends NACK after
        // the DMA reads DR, but by then the slave was already ACK'd on byte 1's
        // 9th clock and begins sending byte 2, leaving RXNE=1 stuck and the bus
        // hung. Fix: abandon DMA, disable ACK now, queue STOP, read via RXNE
        // interrupt.
        push_isr_event(I2C_IsrTag::EV_ADDR_RX_1BYTE);
        disable_DMA_request();
        disableAckBit();
        clear_addr();    // release clock stretch before programming STOP
        generate_stop(); // queued: fires after the single byte is
                         // received
        RegisterUtils::setBits(i2c_->CR2,
                               I2C_CR2_ITBUFEN); // arm RXNE interrupt
      } else {
        push_isr_event(I2C_IsrTag::EV_ADDR_RX, XferSize);
        hdmarx.StartDataStream(reinterpret_cast<uint32_t>(&i2c_->DR),
                               reinterpret_cast<uint32_t>(rxBuffer.data()),
                               XferSize);
        clear_addr(); // must happen after DMA armed
      }
    } else {
  ...
  if (sr1 & I2C_SR1_RXNE) {
    if (snap == i2c_state_t::BUSY_RX &&
      ...
      rxBuffer[0] =
          static_cast<uint8_t>(i2c_->DR); // STOP already queued in ADDR handler
      RegisterUtils::clearBits(i2c_->CR2, I2C_CR2_ITBUFEN);
      disableInterrupt();
      mode_ = i2c_mode_t::NONE;
      dma_complete_.store(false, std::memory_order_relaxed);
      RxEventFlag.store(true, std::memory_order_release);
      state_.store(i2c_state_t::READY, std::memory_order_release);
    }
    return;
  }
  // ── TXE alone: DMA not done yet, do NOT generate STOP ────────────────────
  if (sr1 & I2C_SR1_TXE) {
    if (snap == i2c_state_t::BUSY_TX && dma_complete_) {
...
void DmaI2C::handleMemEventInterrupt(const uint32_t &sr1, const uint32_t &sr2,
                                     const i2c_state_t &snap) {
  push_isr_event(I2C_IsrTag::EV_ENTRY, sr1, sr2);
  // ── SB: Start  generated ────────────────────────────────────────
  if (sr1 & I2C_SR1_SB) {
  ...
  if (sr1 & I2C_SR1_BTF) {
    if (snap == i2c_state_t::BUSY_TX) {
      if (dma_complete_.load(std::memory_order::relaxed)) {
        if (mode_ == i2c_mode_t::MEM_READ) {
          // MEM_READ: register address byte sent — issue repeated START to
          // switch to RX
          push_isr_event(I2C_IsrTag::EV_MEM_RESTART, MemAddr_);
          state_.store(i2c_state_t::BUSY_RX, std::memory_order_relaxed);
          dma_complete_.store(false, std::memory_order_relaxed);
          rxDmaPending_.store(true, std::memory_order_release);
          enable_DMA_request_rx();
          enableAckBit();
          generate_start(); // repeated START — no STOP
        } else {
          if (XferSize == 0) {
            disableInterrupt();
            generate_stop();
...
void DmaI2C::handleEVInterrupt() {
  ...
  const uint32_t sr2 =
      i2c_->SR2; // reading SR2 also helps clear ADDR in some cases
  const i2c_state_t snap = state_.load(std::memory_order::relaxed);

  if (mode_ == i2c_mode_t::MASTER) {
    handleMasterEventInterrupt(sr1, sr2, snap);
  } else if (mode_ == i2c_mode_t::MEM_READ || mode_ == i2c_mode_t::MEM_WRITE) {
...
void DmaI2C::handleERInterrupt() {
  ...
  // Without generate_stop() here the bus stays stuck; without resetting
...
void DmaI2C::handleRxDmaInterrupt() { hdmarx.handleInterrupt(); }
...
void DmaI2C::processRx() {
  drain_isr_log();
  if (RxEventFlag.load(std::memory_order_acquire) == false)
    return;
  RxEventFlag.store(false, std::memory_order_relaxed);
  ...
  onDataReceived();
  if (complete_flag_)
    complete_flag_->store(true, std::memory_order_release);
}
...
void DmaI2C::drain_isr_log() {
  while (isr_log_tail_ != static_cast<uint8_t>(isr_log_head_)) {
    ...
    case I2C_IsrTag::EV_MEM_RESTART:
      LOG_DEBUG("MEM_READ BTF: reg addr {} sent, issuing repeated START",
                (Hex)e.a);
      break;
